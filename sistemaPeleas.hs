...
niñoPueblo = Pers {nombre = "Pepe", felicidad = -1, talkNoJutsu = 0, sanar = 0, luchar = 1, vida = 3.0}
...
finalCombate :: Personaje -> Personaje -> (Bool,String)
finalCombate player enemy
                        | ve <= 0 = (True,"Has ganado el combate")
                        | vp <= 0 = (True, "Has perdido el combate")
                        | otherwise = (False, "")
...
main  = do
        let kal = def {nombre="Kal"}
        let (x,y) = finalCombate kal niñoPueblo
        putStrLn $ show $ (x,y)
        let kal' = modificaStat 5 (-3) kal
        return (finalCombate kal' niñoPueblo)
-}
...
main = do
        ...
        y <- aleatorio
        let x = recibeAtaque kal niñoPueblo y
        putStrLn $ show x
---}
